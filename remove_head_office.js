...
async function cleanup() {
    console.log("Starting cleanup of 'Head Office' contact cards...");
    const q = query(collection(db, 'contact_cards'), where('title', '==', 'Head Office'));
    const snap = await getDocs(q);

    if (snap.empty) {
        console.log("No 'Head Office' cards found in database.");
    } else {
